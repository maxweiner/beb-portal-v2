-- ── Recon commission-label fix — V2 ──
--
-- V1 failed because the Supabase SQL editor wraps multi-statement
-- scripts in a single transaction, so `ON COMMIT DROP` doesn't
-- fire between the two reconciliation_run_match() calls at the end
-- of the file. The 2nd call hits:
--   ERROR 42P07: relation "_tmp_recon_findings" already exists
--
-- Fix: drop the temp table at the top of the function so it's
-- safe to call multiple times in one transaction. Otherwise
-- identical to V1.
-- ============================================================
...
DECLARE
  ...
  v_outstanding INT;
BEGIN
  -- Clear any leftover temp table from a prior call in the same
  -- transaction (Supabase SQL editor batches statements).
  ...
  -- Drop the temp table explicitly too, so back-to-back calls in
  -- the same transaction (e.g. the brand sweep below) don't collide.
  DROP TABLE IF EXISTS _tmp_recon_findings;
...
COMMENT ON FUNCTION public.reconciliation_run_match(TEXT) IS
  'Re-runs reconciliation matching for one brand. Commission label is sourced from buyer_checks.commission_note only; legacy event_days store-commission rows keep their literal label. Safe to call multiple times within one transaction.';

-- Refresh existing findings so the bogus "Store commission" labels disappear.
SELECT public.reconciliation_run_match('beb');
SELECT public.reconciliation_run_match('liberty');

DO $$ BEGIN
  RAISE NOTICE 'Commission-label fix V2 applied + matcher re-run for both brands.';
END $$;
